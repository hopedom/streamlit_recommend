...
class FeaturesEmbedding(Layer):
    ...
    def call(self, x):
        # 들어온 입력의 임베딩을 가져니다.
        offsets = tf.constant(self.offsets, dtype=x.dtype)  # ← 핵심
        x = x + offsets
...
class AutoIntMLP(Layer): 
    ...
    def call(self, inputs, training=False):
        ...
        dnn_embed = tf.reshape(embed_x, shape=(-1, self.embed_output_dim)) # dnn_embed가 Flatten된 형태로 들어가는 PyTorch 구조를 반영
        att_input = embed_x

        # 멀티 헤드 셀프 어텐션 레이어에서 상호작용을 수행합니다. (Interaction 경로)
        for layer in self.int_layers:
            att_input = layer(att_input)

        att_output = Flatten()(att_input)
        att_output = self.final_layer(att_output)
        att_output = tf.nn.relu(att_output)
        
        dnn_output = self.dnn(dnn_embed, training=training)
        
        # 최종 합산 (로짓 반환)
        y_pred = att_output + dnn_output 
...
class AutoIntMLPModel(Model):
    def __init__(self, field_dims, embedding_size, att_layer_num=3, att_head_num=2, 
                 att_res=True, dnn_hidden_units=(32, 32), dnn_activation='relu', l2_reg_dnn=0, l2_reg_embedding=1e-5, dnn_use_bn=False, 
                 dnn_dropout=0.4, init_std=0.0001, **kwargs):
        super(AutoIntMLPModel, self).__init__(**kwargs)
    ...
    def call(self, inputs, training=False):
        # AutoIntMLP 레이어는 최종 합산된 로짓(logit)을 반환합니다.
        logits = self.autoint_layer(inputs, training=training)
        
        # 최종 모델에서 시그모이드 활성화 함수를 적용하여 예측 확률을 반환합니다.
